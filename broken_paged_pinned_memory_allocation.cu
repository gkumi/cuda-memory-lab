...
__host__ std::tuple<float *, float *> allocateRandomHostMemory(int numElements)
{
    ...
    if (!h_a)
    {
        fprintf(stderr, "malloc failed for h_a\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (err != cudaSuccess) 
    { 
        fprintf(stderr, "cudaMallocHost failed for h_b: %s\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE); 
    }
...
__host__ std::tuple<float *, float *, int>readCsv(std::string filename)
{
    ...
    float *h_a = (float *)malloc(size); //pageable
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE
    if (!h_a) throw runtime_error("malloc failed for h_a");
    ...
    float *h_b = nullptr; //pinned
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE
    cudaError_t err = cudaMallocHost((void **)&h_b, size);
    if (err != cudaSuccess) throw runtime_error("cudaMallocHost failed for h_b");
...
__host__ void executeKernel(float *d_a, float *d_b, float *h_c, int numElements, int threadsPerBlock)
{
    ...
    cudaDeviceSynchronize(); //ensure h_c is ready on CPU
}
