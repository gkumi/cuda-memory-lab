...
static float *g_h_a = nullptr;
static float *g_h_b = nullptr;
...
__host__ std::tuple<float *, float *> allocateRandomHostMemory(int numElements)
{
    ...
    if (err != cudaSuccess)
    {
        fprintf(stderr, "cudaHostAlloc failed for h_a: %s\n", cudaGetErrorString(err)); 
        exit(EXIT_FAILURE);
    }
    ...
    if (err != cudaSuccess)
    {
        fprintf(stderr, "cudaHostAlloc failed for h_b: %s\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
...
__host__ std::tuple<float *, float *, int>readCsv(std::string filename)
{
    ...
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE
    cudaError_t err = cudaHostAlloc((void **)&h_a, size, cudaHostAllocMapped);
    if (err != cudaSuccess) throw runtime_error("cudaHostAlloc failed for h_a");
    ...
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE
    err = cudaHostAlloc((void **)&h_b, size, cudaHostAllocMapped);
    if (err != cudaSuccess) throw runtime_error("cudaHostAlloc failed for h_b");
...
__host__ std::tuple<float *, float *> allocateDeviceMemory(int numElements)
{
    ...
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE
    cudaError_t err = cudaHostGetDevicePointer((void **)&d_a, (void *)g_h_a, 0);
    if (err != cudaSuccess)
    {
    ...
    float *d_b;
    // FILL IN HOST AND DEVICE MEMORY ALLOCATION CODE
    err = cudaHostGetDevicePointer((void **)&d_b, (void *)g_h_b, 0);
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to allocate device vector d_a (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
...
__host__ void deallocateMemory(float *d_a, float *d_b)
{
    ...
    if (g_h_a) cudaFreeHost(g_h_a);
    if (g_h_b) cudaFreeHost(g_h_b);
    g_h_a = nullptr;
    g_h_b = nullptr;
